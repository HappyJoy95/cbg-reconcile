...
前端点「立即运行」→ 起一个子进程 → 轮询 `/api/run` 拿实时日志。
...
class RunManager:
    ...
    # ---------------------------------------------------------------- 启动
    def start(self, root: Path, config: str, *, days_ago: int | None = 1,
              date: str | None = None, lookback: int | None = None,
              lookahead: int | None = None) -> RunJob:
        with self._lock:
            if self.current():
                raise RuntimeError("已经有一个对账在跑了，等它结束")
            argv = [sys.executable or "python", "-u", "-m", "src.cli", "-c", config, "check"]
            if date:
                argv += ["--date", date]
