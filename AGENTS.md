...
* ⭐ **开工前先写「开发目标」**：`.dsh/docs/<日期>-<版本>-开发目标.md`。
  ...
  **本版不做的** / 进展）。其中 **「本版不做的」是必填** —— 那一节就是
  防范围膨胀的闸门。
  ⚠ **中途冒出来的新想法，先对一眼那份文档**：不是这版的就记进去、
  留到下一版，别顺手做掉。
  * 2.0.0 写了 → 范围清清楚楚。
  * **2.1.0 没写** → 一版装了 **11 件事，其中 5 件跟主线无关**
    （复盘在 `2026-09-18-2.1.0-开发目标.md` 第八节）。
    更糟的是那份 2.0.0 的文档**当时没被任何地方引用**，
    下一个会话根本不知道有这规矩 —— 所以这条现在写在这儿。
* **改完一条先别急着 push、别急着升版本号。** 攒着，等用户说"推吧"。
* ⚠ **beta 包也别自己主动打。**（2026-09-18 用户纠正：「你封 beta 有点快啊，
  我还没搞完呢」）
  用户**还在逐条看**的时候，每改一条就打一个包是**白打**：那一版马上会被
  下一个改动取代，dist 里堆一串几乎一样的 zip，用户反而不知道该拿哪个。
  → **等用户说"打个包我试试"再打。** 中途改多少条都先攒在工作区。
  （真要打时：`bash tools/build_package.sh beta`，自动编号 `beta0/beta1/…`。）
